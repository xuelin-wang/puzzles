 ...
 * If Ai is -1, it means you can’t jump to the place indexed i in the array.
 ...
 DP 1:
     DP(i): min cost to jump from i to end of A.

 DP 2:
     DP(i): at most i jumps to index N. the source/cost map.
     DP(i+1): for all indexes i: for each par (s, c) inb DP(i) where s is one jump reachable,
          put (i, min(cost(i, s) + c) for all (s,c))
 Cost: O(nk)
